...
class YouTube:
    ...
    async def get_related(
        ...
        if not related and context_query:
            # Cloud YouTube search can return candidates without recognizable
            # language labels. Retry once without the language constraint so
            # autoplay does not stop after only 1-2 tracks. Duplicate checks
            # remain active in the fallback.
            related = await self._related_from_search(
                current, played, played_titles, context_query=None
            )
        if related:
