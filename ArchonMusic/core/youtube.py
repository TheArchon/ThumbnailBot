...
class YouTube:
    ...
    async def stream_url(self, video_id: str, video: bool = False):
        """Stable stream resolver for the call layer."""
        try:
            return await self.download(video_id, video=video)
        except Exception as e:
            logger.warning(f"[YouTube] stream_url failed for {video_id}: {e!r}")
            return None
    ...
    async def autoplay_track(
        ...
        """Stable autoplay API. Always returns a Track-like object or None."""
        exclude = {str(x) for x in (exclude or set())}
        exclude_titles = {
            str(x).strip().lower() for x in (exclude_titles or set())
        }

        try:
            try:
                result = await self.get_related(
                    video_id, video=video, max_duration=900
                )
            except TypeError:
                # Compatibility with older implementations.
                result = await self.get_related(video_id, 900)

            if result is not None and hasattr(result, "id"):
                if str(result.id) not in exclude:
                    rt = str(getattr(result, "title", "") or "").strip().lower()
                    if not rt or rt not in exclude_titles:
                        # Recommendation result is preferred. If the title
                        # itself exposes another language, don't switch away
                        # from the source language.
                        if language:
                            language_tokens = {
                                "bhojpuri": ("bhojpuri", "भोजपुरी"),
                                "hindi": ("hindi", "हिंदी", "bollywood"),
                            ...
                            if tokens and not any(t in rt for t in tokens):
                                result = None
                            else:
                                return result
                        else:
            ...
            logger.warning(f"[YouTube] related autoplay failed: {e!r}")

        if title:
            try:
                search_title = title
                if language:
                    search_title = f"{title} {language} song"
                result = await self.search(search_title, 0, video=video)
                if result is not None and hasattr(result, "id"):
                    if str(result.id) not in exclude:
                        result_title = str(
                        ...
                        tokens = language_tokens.get(language, ())
                        if not tokens or any(t in result_title for t in tokens):
                            return result
            except Exception as e:
                logger.warning(f"[YouTube] title autoplay fallback failed: {e!r}")
            except Exception as e:
                logger.warning(f"[YouTube] title autoplay fallback failed: {e!r}")
