...
async def play_hndlr(
    ...
    if not file.file_path and getattr(file, "id", None):
        # Start the API stream/download immediately after search resolution.
        # stream_url() deduplicates concurrent requests and returns a direct
        # provider URL immediately when the API supports it.
        import asyncio
        download_task = asyncio.create_task(yt.stream_url(file.id, video=video))
